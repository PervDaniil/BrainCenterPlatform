...
export default function QuizGame({ params }: { params: { level: string; topic: string } }) {
  ...
  async function fetchQuestions() {
    setLoading(true);
    try {
      const quizes = await fetchQuizQuestions(level);
      setQuestions(quizes);
    } catch (err) {
      console.error(err);
  ...
  return (
    <main className="w-full h-screen relative overflow-hidden grid items-center">
      <AnimatePresence>
        {showAnimation && (
          <motion.div
